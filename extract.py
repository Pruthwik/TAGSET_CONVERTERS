...
class Converter() :
		...
		for scheme in mapData.childNodes : 
			for rule in scheme.childNodes :
				if rule.childNodes!=() :
					...
					if expression!=[] :
						#print tuple(sourceTag)
						mapSet[tuple(sourceTag)]=expression
					else : 
						#print tuple(sourceTag)
	...
	def LoadWordlist(self,filePath) :
		...
		for fileName in targetFiles : 
			wordList=[]
			listFile = codecs.open(filePath + fileName , encoding="utf-8" , mode="r")
			for word in listFile : 
				if word!="" :
					wordList.append((word.strip()).encode("utf-8").decode("utf-8"))
	...
	def ExpressionRecognizer(self,sourceTag,expression) :
		...
		tagNameDict={}
		if len(leftSideSplit)>self.maxQueueLength+1 : print "Max Length is ", self.maxQueueLength ; return		
		rightSideSplit=rightSide.split("-")
